...
from helper_functions import get_adc_gains, get_frequency, get_leads, load_recording, load_header, \
    create_signal_dictionary, samples_to_volts, standardize_leads, write_wfdb_file

format_4_by_3 = [["I", "II", "III"], ["aVR", "aVL", "aVF", "AVR", "AVL", "AVF"], ["V1", "V2", "V3"], ["V4", "V5", "V6"]]

# Run script.
def get_paper_ecg(input_file, header_file, output_directory, seed, add_dc_pulse, add_bw, show_grid, add_print,
                  start_index=-1, store_configs=False, store_text_bbox=True, key='val', resolution=100, units='inches',
                  papersize='', add_lead_names=True, pad_inches=1,
                    ...
                    if (full_mode != 'None' and key == full_mode):
                        if (len(record_dict[key][start:]) > int(rate * 10)):
                            ...
                            if 'full' + full_mode not in segmented_ecg_data.keys():
                                ...
                                segmented_ecg_data['full' + full_mode] = segmented_ecg_data['full' + full_mode] + frame[
                                    'full' + full_mode].tolist()
                        else:
                            frame['full' + full_mode] = samples_to_volts(record_dict[key][start:], adc[gain_index])
                            frame['full' + full_mode] = center_function(frame['full' + full_mode])
                            if 'full' + full_mode not in segmented_ecg_data.keys():
    ...
    for i in range(len(ecg_frame)):
        ...
        x_grid, y_grid = ecg_plot(ecg_frame[i], full_header_file=full_header_file, style=grid_colour, sample_rate=rate,
                                  ...
                                  full_mode=full_mode, store_text_bbox=store_text_bbox, show_lead_name=add_lead_names,
                                  show_dc_pulse=dc, papersize=papersize, show_grid=(grid),
                                  standard_colours=standard_colours, bbox=bbox, print_txt=print_txt)
